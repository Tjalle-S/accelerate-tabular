--
-- Implementation from:
-- https://github.com/diku-dk/CFAL-bench/blob/main/quickhull/accelerate/src/Quickhull.hs
-- See also the paper: van Balen, David, et al. "Comparing parallel functional array languages: programming and performance." arXiv preprint arXiv:2505.08906 (2025).
--

module Quickhull.Accelerate (quickhull, Point) where
...
initialize points = T2 newPoints hull
  where
    ...
    line@(T2 leftMost rightMost) =
      the $
        fold1 (\(T2 min1 max1) (T2 min2 max2) -> T2 (min min1 min2) (max max1 max2)) $
          map (\p -> T2 p p) points
  ...
  where
    ...
    newHull =
      permute
        ...
        ( \idx ->
            cond (furthestPerLabel ! idx == noPoint) Nothing_ $
              Just_ $
                I1 $
                  (hullNewIndices ! idx) + 1
        )
        furthestPerLabel
        ...
        f (T2 p label) =
          cond (p == f) 0 $
            cond (nonNormalizedDistance (T2 a f) p > 0.0) 1 $
              cond
                (nonNormalizedDistance (T2 f b) p > 0.0)
                2
                0
          where
            a = hull !! label
    ...
    destination idx =
      cond (g == 1) (Just_ $ I1 $ segmentOffset + leftOffset - 1) $
        cond
          (g == 2)
          (Just_ $ I1 $ segmentOffset + leftSize + rightOffset - 1)
          Nothing_
      where
        g = groups ! idx
        label = labels ! idx
