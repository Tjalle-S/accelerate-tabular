...
data SliceDescriptor' key desc where
  ...
  SliceKeep :: (SliceDescriptor keys desc, Eq key)
            => SliceDescriptor' keys desc
            -> SliceDescriptor' (keys :. key) (desc :. Keep)
  SliceIndex :: (SliceDescriptor keys desc, Eq key)
             => SliceDescriptor' keys desc
             -> Exp key
...
type family SliceResult t desc where
  SliceResult Z             Z                    = Z
  SliceResult (tail :. key) (desc :. Keep)       = SliceResult tail desc :. key
  SliceResult (tail :. key) (desc :. SliceFix _) = SliceResult tail desc
