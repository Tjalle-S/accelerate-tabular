...
module Data.Array.Accelerate.Tabular.Util.SegmentedSort (
  segmentedSort
, segmentedSortBy
) where
...
segmentedSort :: (Ord a)
              ...
              -> Acc (Vector a)
              -> Acc (Vector a)
segmentedSort = segmentedSortBy compare

segmentedSortBy :: (Elt a)
                => (Exp a -> Exp a -> Exp Ordering)
                -> Acc (Vector Bool)
                -> Acc (Vector a)
                -> Acc (Vector a)
segmentedSortBy cmp flags vals = result
  where
    -- We stop when each segment contains just one element, as segments of
    -- one element are sorted.
    T2 result _ = awhile condition (step cmp) $ T2 vals flags
