...
import Data.Array.Accelerate                                        ( Acc, Arrays, Array, Shape, Elt, DIM0, DIM1, DIM2, DIM3, Z(..), (:.)(..), fromList, Vector )
...
genAssocs :: (Elt key, Elt val, Eq key) => Gen key -> Gen val -> DIM1 -> Gen (Vector (key, val))
genAssocs k v (Z :. len) = do
  ks <- nub <$> G.list (R.singleton len)  k
  let len' = length ks
  vs <-         G.list (R.singleton len') v
  pure $ fromList (Z :. len') (zip ks vs)
...
deriving instance (Ord tail, Ord head) => Ord (tail :. head)
